...
class FraudDetectionsController extends BaseController
{
    ...
    {
        if(empty($request->filter_start) or $request->filter_start == "Y-m-d"){
            $eloq = FunnelOrder::selectRaw('funnel_orders.id, funnel_orders.shipping_method_service_cost, funnel_orders.created_at, CONCAT(billing_address_first_name, billing_address_first_name) AS full_name, funnel_orders.status,funnels.title,funnel_orders.fraud')
