...
@section('title', 'Report Coin Statistic')

@section('page-header', 'Report Coin Statistic')
...
@section('breadcrumb')
	<ol class="breadcrumbs">
	  ...
	  <li><span>Report Coin Statistic</span></li>
	</ol>
@endsection
...
@section('content')
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-2">
                        <select data-plugin-selectTwo class="form-control" onchange="location = this.value;">                                               
                                <option value="{{ route('admin-hq-coin-statistic', ['filter' => "year"]) }}" @if($filter == "year") selected @endif>Year</option>
                                <option value="{{ route('admin-hq-coin-statistic', ['filter' => "month"]) }}" @if($filter == "month") selected @endif>Month</option>
                                <option value="{{ route('admin-hq-coin-statistic', ['filter' => "day"]) }}" @if($filter == "day") selected @endif>Day</option>                                                    
                        </select>
                    </div>
                </div>
            </form>
    ...
    <div class="panel panel-primary">
        <div class="panel-heading"><h3 class="panel-title">Coin</h3></div>
        <div class="panel-body">
...
$('html').addClass(' sidebar-left-collapsed');
       ...
       var list_commission_table = $('#list-transactions-table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url : "{!! route('admin-datatables-coin-statistic') !!}",
                data: function(d){
