...
@section('content')
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <table id="list-non-active-member-table" class="table table-hover table-bordered table-condensed table-responsive" data-tables="true">
                    <thead>
                        <tr>
                            <th class="center-align">Member ID</th>
                            <th class="center-align">Name</th>
    ...
    onConfirm: function() {
       ...
       function getOptionRecord(id){
            $.ajax({
                ...
                success: function(response)
                {
                    $("[name='to_user_id']").html(response.getOptionRecord);
        ...
        function show_form_spillover(id){
            ...
            $(".select2-single").val('all').change();
            $(document.body).on("change",".select2-single",function(){
                getOptionRecord($("[name='user_id']").val(),this.value);
                $("#select2").select2("val", "");
            });
            $.ajax({
                type: "POST",
